...
def apply_ui_style():
    st.markdown(f"""
        <style>
            ...
            .card-title {{ font-size: 28px; font-weight: 800; letter-spacing: -1px; line-height: 1.2; }}
            ...
            .hero-title {{ font-weight: 900; font-size: 48px; line-height: 1.1; letter-spacing: -2px; }}
...
if st.session_state.view == 'home':
    ...
    hero_url = f"data:image/jpeg;base64,{img_forest}" if img_forest else ""
    
    st.markdown(f"""
    <div class="hero-section" style="background-image: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.4)), url('{hero_url}');">
    ...
    if member_data:
        ...
        if selected_group != "조를 선택해 주세요":
            st.markdown(f'<div class="info-box"><b>{selected_group} 명단</b><br>{member_data[selected_group]}</div>', unsafe_allow_html=True)

    st.markdown("#### 🗺️ 산책로 주요 지점")
