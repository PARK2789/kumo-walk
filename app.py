...
# 파일명 매칭 (저장소에 아래 이름의 파일들이 있어야 합니다)
...
<style>
    ...
    .hero-sub {{ font-size: 20px; opacity: 0.95; margin-top: 15px; font-weight: 400; }}
    ...
    .prog-tag {{ font-size: 13px; font-weight: 700; color: #007AFF; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 1px; }}
...
# 5. 데이터 및 정교한 올레길 실제 경로 (네이버 지도 동기화)
olle_fine_path = [
    [36.111006, 128.313156], [36.1115, 128.3128], [36.1122, 128.3126], [36.1132, 128.3125], 
    [36.1141, 128.3126], [36.1151, 128.3127], [36.1160, 128.3128], [36.1168, 128.3128], 
    [36.1176, 128.3129], [36.1185, 128.3131], [36.1192, 128.3136], [36.1196, 128.3141],
    [36.119797, 128.314458], # Activity 1: 목표달성 'Goal-In'
    [36.1199, 128.3149], [36.1200, 128.3155], [36.1201, 128.3163], [36.1202, 128.3170],
    [36.1202, 128.3178], [36.1201, 128.3186], [36.1198, 128.3195], [36.119397, 128.319959], # Activity 2: Bottleneck 타파
    [36.1189, 128.3198], [36.1184, 128.3197], [36.1178, 128.3195], [36.1171, 128.3193], 
    [36.1163, 128.3189], [36.1157, 128.3184], [36.1150, 128.3178], [36.1144, 128.3172], 
...
if st.session_state.page == 'main':
    ...
    # [2] 정밀 지도 섹션 (실제 올레길 굴곡 반영)
    st.markdown("#### 🗺️ 올레길 산책 코스")
    st.caption("파란색 실선이 지도의 '금오산 올레길' 산책로를 그대로 따라가는 실제 경로입니다.")
    
    # CartoDB Voyager 스타일
    m = folium.Map(location=[36.1155, 128.3160], zoom_start=15, tiles="cartodbvoyager")
    
    # 실제 산책로를 정교하게 하이라이트 (진한 블루 라인)
    folium.PolyLine(
        locations=olle_fine_path,
        color="#007AFF",
        weight=6,
        opacity=0.8
    ).add_to(m)
